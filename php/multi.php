...
switch((int) $_POST['cmd']) {
    case "CMD_REQUEST_LOGIN":
        if( empty($_POST['id'] || $_POST['pw']) ){
            ...
            $id = addslashes($id);
            $id = stripslashes($id);
            $id = mysql_real_escape_string($conn,$id);
            $pw = addslashes($pw);
            $pw = stripslashes($pw);
            $pw = mysql_real_escape_string($conn,$pw);
            ...
            if($row){
                ...
                die('<script>alert("existing account");</script>');
            }
    ...
    case "CMD_REQUEST_REG":
        if( empty($_POST['id'] || $_POST['pw'] || $_POST['name'] || $_POST['pw_re'] ) ){
            ...
            $name = mysql_real_escape_string($conn,$name);
            $id = addslashes($id);
            $id = stripslashes($id);
            $id = mysql_real_escape_string($conn,$id);
            $pw = addslashes($pw);
            ...
            $pw_re = mysql_real_escape_string($conn,$pw_re);
